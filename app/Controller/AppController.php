...
class AppController extends Controller {
	...
	public function beforeFilter() {
	    ...
	    if (isset($this->params['prefix']) && $this->params['prefix'] == 'admin' && $this->isAuthorized()) {
			...
			$statistics['Usuarios'] = $this->User->find('count');
			$statistics['Proyectos'] = $this->Project->find('count');
			$statistics['Donaciones'] = $this->Contribution->query("SELECT ROUND(SUM(amount), 2) as total FROM contributions")[0][0]['total'];
			$statistics['Usuarios Inactivos'] = $this->User->find('count', array(
				'conditions' => array(
					'User.is_active' => false
				)
			));
			$statistics['Proyectos Inactivos'] = $this->Project->find('count', array(
				'conditions' => array(
